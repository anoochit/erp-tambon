...
//-----------แสดงข้อมูล-------------------
    ...
    <td height="30"  colspan="6"  style="border: #000000 1px solid;">
      <div align="left">&nbsp;&nbsp;&nbsp;[ <a href="?action=add_type_mem" class="catLink1">	เพิ่มข้อมูลประเภทผู้ขอใช้บริการ</a> ]</td> 
...
/*
	...
	for($i=1;$i<=$page;$i++){
		if($_GET['page']==$i){ //ถ้าตัวแปล page ตรง กับ เลขที่วนได้
			echo "[<a href='?action=add_user1&start=".$limit*($i-1)."&page=$i&div_name=$div_name&sub_name=$sub_name&level1=$level1&status=$status'> $i </A>]"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 1
		}else{
			echo "[ <a href='?action=add_user1&start=".$limit*($i-1)."&page=$i&div_name=$div_name&sub_name=$sub_name&level1=$level1&status=$status'> $i </A> ]"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 2
		}
	}
...
<!--  </td>
   ...
   <? // }elseif($total <=0){?>
     <!--<tr class="lgBar"><td colspan="10" style="border: #000000 1px solid;"  height="30" align="center"> ไม่พบข้อมูล</td>
 </tr> -->
 <? // }?>
	 </table>
