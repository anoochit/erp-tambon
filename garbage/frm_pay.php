...
if($save_as <>''){
			if($total <> '' and $chk <>''){
				for ($i=0;$i<=$total;$i++) {
					if ($chk[$i] != "") { 
							$k = explode("*",$chk[$i]);
							$sql_insert="update receive  set rec_status ='ชำระแล้ว' , p_date = '".date_format_sql($pdate)."' where mcode = '".$k[0]. "' and  rec_id ='".$k[1]. "'  ";
...
$sql_ex  .=  "  and rec_status = 'ค้างชำระ' and c.MCODE = b.MCODE ";
