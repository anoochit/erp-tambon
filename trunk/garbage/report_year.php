 ...
 <table  width="100%"   border="0" align="center" cellpadding="1" cellspacing="1">
  <tr>
    <td  colspan="2" style="border: #7292B8 1px solid;"  ><table width="100%" align="center" cellspacing="1"  cellpadding="1" border="0">
      <tr class="lgBar">
        <td  height="28" colspan="14" bgcolor="#DEE4EB"><div  align="left">&nbsp;&nbsp;<strong>รายงานสรุปยอดตั้งเก็บและจัดเก็บได้ค่าธรรมเนียมขยะมูลฝอยประจำปี</strong></div></td>
      </tr>
      <tr class="bmText"  bgcolor="#DEE4EB">
        <td width="13%"  height="31" align="center"><strong>เดือน</strong></td>
        ...
        <td width="13%"  align="center"><strong>ยอดค้างยกไป</strong></td>
		 <td width="13%"  align="center"><strong>เก็บได้ในเดือน</strong></td>
		  <td width="13%"  align="center"><strong>ค้างเก่าเก็บได้</strong></td>
		  <td width="10%"  align="center"><strong>จำนวนราย</strong></td>
      </tr>
<?
if($search <>'')
//$p_date_1 = ($year-543)."-".($month+1)."-";
$SumMtotal = 0;
$stotal3 = 0; 
$stotal = 0;
$stotal4 = 0;
...
<!--///////////////////////09 ก.ย.//////////////////////////// -->
 ...
 ?>
		...
		echo number_format($Sum0); 
		$b1=$Sum0;?></td>
		<td  height="25"   align="center">&nbsp;<? 
		$ff2 = explode("*",Find_SumTotalG8_2($year,$month));
		echo number_format($ff2[0]); 
		$c1= $ff2[0];
		?></td>
		<td  height="25"  align="center">&nbsp;<? echo number_format(Find_SumRemain8_2($year,$month));
		$d1=Find_SumRemain8_2($year,$month);?></td>
		<td  align="center">&nbsp;<? 
		$Sume1 = Find_SumPayArrearsG8_2($year,$month)+ Find_SumGeTG8_2($year,$month);
		echo number_format($Sume1);
		$e1=$Sume1;?></td>
        <td  align="center">&nbsp;<?
		...
		$f1=$Sum4;
		?> </td>
	    <td  height="25"   align="center">&nbsp;<? echo number_format(Find_SumGeTG8_2($year,$month));
		$g1=Find_SumGeTG8_2($year,$month); ?></td>
		<td  height="25"  align="center">&nbsp;<? echo number_format(Find_SumPayArrearsG8_2($year,$month)); 
 ...
 ?>
        ...
        <td  height="25"   align="center">&nbsp;<? echo number_format(Find_SumRemain8($year,$month)); 
		...
		$f2=$Sum3;
		?> </td>
	    <td  height="25"   align="center">&nbsp;<? echo number_format(Find_SumGeTG8($year,$month));
		$g2=Find_SumGeTG8($year,$month);  ?></td>
		<td  height="25"  align="center">&nbsp;<? echo number_format(Find_SumPayArrearsG8($year,$month));
...
<tr class="bmText" bgcolor="<? echo $bg?>" onMouseOver= "if (typeof(this.style) != 'undefined') this.style.backgroundColor = '#b9c9fe'" onMouseOut="if (typeof(this.style) != 'undefined') this.style.backgroundColor = ''">
		...
		<td  height="25"   align="center">&nbsp;<?$ff = explode("*",Find_SumTotalG8($year,$month));
		echo number_format($ff[0]); 
		$c5=$ff[0];?></td>
		<td  height="25"  align="center">&nbsp;<? 
		$Sum1 =(Find_SumRemain8($year,$month)+$ff[0]);
...
<tr class="bmText" bgcolor="<? echo $bg?>" onMouseOver= "if (typeof(this.style) != 'undefined') this.style.backgroundColor = '#b9c9fe'" onMouseOut="if (typeof(this.style) != 'undefined') this.style.backgroundColor = ''">
 ...
 ?>
      ...
      <tr class="bmText" bgcolor="#DEE4EB"  onMouseOver= "if (typeof(this.style) != 'undefined') this.style.backgroundColor = '#b9c9fe' " onMouseOut="if (typeof(this.style) != 'undefined') this.style.backgroundColor = '' ">
        ...
        <td  height="25"   align="center">&nbsp;<?=number_format($SumB)?></td>
        <td  height="25"  align="center">&nbsp;<?=number_format($SumC)?></td>
        <td  align="center">&nbsp;<?=number_format($SumD)?></td>
		<td  align="center"><?=number_format($SumE)?></td>
        <td  align="center">&nbsp;<?=number_format($SumF)?></td>
        <td  align="center">&nbsp;<?=number_format($SumG)?></td>
        <td  align="center">&nbsp;<?=number_format($SumH)?></td>
        <td  align="center">&nbsp;<?=number_format($SumI)?></td>
	      </tr>
    </table></td>
    </tr>
  </table>
...
<div align="center">
<input  type="button" name="print" value=" พิมพ์ "  onclick="window.open('report9.php?month=<?=$month?>&year=<?=$year?>&year2=<?=$Yr?>')"/ class="buttom">
...
function Find_SumRemain8($year,$month){
	if ($month == '09' or $month == '10' or $month == '11' or $month == '12') {
        ...
        $sql =  "select  if(sum(total) is null,'0.00',sum(total)) as total1 from receive 
...
function Find_SumGeTG8($year,$month){
	...
	$month1 = ($month+1);
	 ...
	 $sql1 =  "select count(rec_id), sum(total) as total1 from receive m
...
function Find_SumPayArrearsG8($year,$month){
	...
	$month2 = ($month+1);
		...
		$sql2 =  " select count(rec_id) ,sum(total) as total2 from receive m  
...
function Find_SumTotalG8($year,$month){
        $sql3 =  "select if(sum(total) is null,'0.00',sum(total)) as  total3 ,count(rec_id) as crec_id  from receive
		            where myear =  '".$year. "' and monthh =  ".$month." 
...
function Find_SumGeTG8_2($year,$month){
	if ($month == '09' or $month == '10' or $month == '11') {
			...
			$p_date_5 = ($Yr-543)."-10-%";
        $sql5 =  "select count(rec_id), sum(total) as total5 from receive m
...
function Find_SumPayArrearsG8_2($year,$month){
	...
	$p_date_6 =  ($Yr-543)."-10-%";
	$sql6 =  " select count(rec_id) ,sum(total) as total6 from receive m  
...
function Find_SumRemain8_2($year,$month){
	...
	$sql7 =  " select if(sum(total) is null,'0.00',sum(total)) as total7 from receive m  
...
function Find_SumTotalG8_2($year,$month){
        $sql8 =  "select if(sum(total) is null,'0.00',sum(total)) as  total8 ,count(rec_id) as crec_id  from receive
