...
<tr class="header" bgcolor="#DEE4EB">
      <td  height="28" colspan="14"><div  align="left">&nbsp;&nbsp;รายงานรายชื่อผู้ยกเลิกใช้บริการจัดเก็บขยะมูลฝอย ประจำเดือน  <?=mounth3($month)?>&nbsp;<?=$year?></div></td>
        </tr>
  <tr class="body1"  bgcolor="#DEE4EB">
		...
		<td width="12%"  height="31" align="center"><p><strong>เลขทะเบียน</strong><strong></strong></p></td>
         <td width="19%"  align="center"><p><strong>วันที่ขอหยุดใช้</strong></p></td>
...
$sql_1 ="Select  q.MCODE,concat(pren,name,'  ',surn) as name,moo,HNO1,
 ...
 Where q.mem_id = m.mem_id  and q.MCODE = c.MCODE and q.hocode = h.hocode and cost = qty
  and q.pvcode = h.pvcode and q.amcode = h.amcode and q.tucode = h.tucode 
...
<td width="36%"  height="25"  align="left"><strong>&nbsp;&nbsp;จำนวนผู้ขอยกเลิก :: &nbsp; <?=$i ?> &nbsp; ราย</strong></td>
<td width="64%"  height="25"  align="left"><strong>&nbsp;&nbsp;รวมเป็นเงิน :: &nbsp; <?=number_format($total_qty) ?> &nbsp;บาท</strong></td>
</tr>
</table>	
	  </td>
    </tr>
