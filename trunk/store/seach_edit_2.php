...
<tr class="bmText">
	...
	<td  class="lgBar1" height="25"  >
		<div > <img src="images/Search.png" align="absmiddle">&nbsp;&nbsp;เบิกพัสดุ &nbsp;&nbsp; [ <a href="?action=new_buy_2_1" class="catLink">เพิ่มเบิกพัสดุ </a>]</div>	</td>
	</tr>
</table>
	</td>
	</tr>
...
WHERE 1 = 1   ";
$sql .= " AND p.div_id = '$_SESSION[div_id]'  ";
if($pay_id  <> '' ){
...
<tr class="lgBar">
      <td  height="28" colspan="6"><div  align="left">&nbsp;&nbsp;แก้ไข / ลบ เบิกพัสดุ</div></td>
          </tr>
  <tr class="bmText"  bgcolor="#DEE4EB">
        <td width="11%"  height="31"><div align="center"><strong>เลขที่เบิก</strong></div></td>
		<td width="15%"  height="31"><div align="center"><strong>ทะเบียนเลขที่เบิก</strong></div></td>
         <td width="10%" ><div align="center"><strong>วันที่เบิก</strong></div></td>
		<td width="20%"  align="center"><strong>หน่วยงาน / ชื่อผู้เบิก</strong></td> 
         <td width="34%" ><div align="center"><strong>ชื่อพัสดุ</strong></div></td>
		 <td width="10%" >&nbsp;</td> 
          </tr>
<?
$i = 0;
...
?>       
<a href="?action=view_detail_2&p_id=<?=$rs[p_id1]?>&pd_id=<?=$rs[pd_id1]?>"  >  
<tr class="bmText" bgcolor="<?=$bg?>" onmouseover= "if (typeof(this.style) != 'undefined') this.style.backgroundColor = '#b9c9fe'" onMouseOut="if (typeof(this.style) != 'undefined') this.style.backgroundColor = ''">
...
if($_SESSION[div_id]=='0'){
 ...
 <td  align="center"><a href="?action=view_detail_2&p_id=<?=$rs[p_id1]?>&pd_id=<?=$rs[pd_id1]?>"  >   แก้ไข / ลบ</td></tr></a> 
...
<? /* สร้างปุ่มย้อนกลับ */
if($Prev_Page) 
echo " <a href='$PHP_SELF?action=seach_edit_2&search=$search&Page=$Prev_Page&pay_id=$pay_id&code1=$code1&pay_date=$pay_date&rd_name=$rd_name&type_id=$type_id&p_id=$p_id'><< ย้อนกลับ </a>";
for($i=1; $i<$Num_Pages; $i++){
if($i != $Page)

echo "[<a href='$PHP_SELF?action=seach_edit_2&search=$search&Page=$i&pay_id=$pay_id&code1=$code1&pay_date=$pay_date&rd_name=$rd_name&type_id=$type_id&p_id=$p_id'>$i</a>]";
else 
echo "<b> $i </b>";
}
/*สร้างปุ่มเดินหน้า */
if($Page!=$Num_Pages)
echo "<a href ='$PHP_SELF?action=seach_edit_2&search=$search&Page=$Next_Page&pay_id=$pay_id&code1=$code1&pay_date=$pay_date&rd_name=$rd_name&type_id=$type_id&p_id=$p_id'> หน้าถัดไป>> </a>";
