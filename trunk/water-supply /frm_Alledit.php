		...
		if($type ==0){
						if($total <> '' and $chk <>''){ //แก้ไขวันที่ออกใบเสร็จ
							for ($i=0;$i<=$total;$i++) {
								if ($chk[$i] != "") { 
										$k = explode("*",$chk[$i]);
										$sql_insert="update meter  set m_date = '".date_format_sql($RDATE)."' where mcode = '".$k[0]. "' and  rec_id ='".$k[1]. "'  ";
						...
						if($total <> '' and $chk <>''){ //แก้ไขวันที่ออกใบเสร็จ
							for ($i=0;$i<=$total;$i++) {
								if ($chk[$i] != "") { 
										$k = explode("*",$chk[$i]);
										$sql_insert="update meter  set rec_date = '".date_format_sql($RDATE)."' where mcode = '".$k[0]. "' and  rec_id ='".$k[1]. "'  ";
										$result_insert  = mysql_query($sql_insert); 
								}
							}	
						}
					...
					if($total <> '' and $chk <>''){
							for ($i=0;$i<=$total;$i++) {
								if ($chk[$i] != "") { 
										$k = explode("*",$chk[$i]);
										$sql_insert="update meter  set p_date = '".date_format_sql($RDATE)."' where mcode = '".$k[0]. "' and  rec_id ='".$k[1]. "'  ";
										$result_insert  = mysql_query($sql_insert); 
								}
							}	
						}
					...
					if($total <> '' and $chk <>''){
							for ($i=0;$i<=$total;$i++) {
								if ($chk[$i] != "") { 
										$k = explode("*",$chk[$i]);
										$sql_insert="update meter  set rec_status ='ค้างชำระ' ,  p_date ='1111-11-11'  where mcode = '".$k[0]. "' and  rec_id ='".$k[1]. "'  ";
										$result_insert  = mysql_query($sql_insert); 
								}
							}	
						}
...
if($ex =='0') $monthh = substr($month,1);	
                ...
                $sql_ex  .= "  rec_status,if(rec_id is null,'',rec_id) as rec_id ,rec_date,";
                $sql_ex  .= "  amount , m_date,if(m_rate is null,'',m_rate) as m_rate,if(m_amt is null,'',m_amt) as m_amt , m2.sum_m , m2.p_date";
                $sql_ex  .= "  from member m,q_water q, meter m2  ";
                $sql_ex  .= "  Where q.mem_id = m.mem_id ";
                $sql_ex  .= " and q.MCODE = m2.MCODE";
    			$sql_ex  .=  "  and monthh = '" .$monthh. "' and myear = '" .$year. "'  and  rec_status  is not null  ";
...
<?  if($Num_Rows >0){?>
  ...
  <td  height="35"  align="left" colspan="9"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
  ...
  <tr>
    <td width="70%"><table name="add" cellpadding="1" cellspacing="1" border="0" width="100%">
			...
			<select name="type" onChange="dochange1('Z1', this.value)" >
			  ...
			  <option value="0" <? //if($type =='1') echo "selected"?> >แก้ไขวันที่กรอกมาตร </option>
...
<? }?>
  ...
  <td width="7%"  height="31" align="center"><strong>เลือกทั้งหมด<br>
  <input name="allbox" onClick="selectall();" type="checkbox">
		...
		<td width="10%"  align="center"><strong>เลขที่ใบเสร็จ</strong></td>
		 <td width="12%"  align="center"><strong>วันที่ออกใบเสร็จ</strong></td>
		<td width="8%"  align="center"><strong>รวมเงิน</strong></td>
         <td width="10%"  align="center"><p><strong>สถานะ</strong></p></td>
		 <td width="16%"  align="center"><p><strong>วันที่ชำระเงิน</strong></p></td>
          </tr>
<?
if($Page >= 2 ){
			$i=$Page_start ;
...
<tr class="bmText" bgcolor="<? echo $bg?>" onmouseover= "if (typeof(this.style) != 'undefined') this.style.backgroundColor = '#b9c9fe'" onMouseOut="if (typeof(this.style) != 'undefined') this.style.backgroundColor = ''">
 ...
 if($rs_1[rec_id] =='') echo date_2(date("Y-m-d"));
   ...
   ?> </div></td>
 <td > <div   align="right">&nbsp; <? echo number_format($rs_1[sum_m],2);  ?>  &nbsp; &nbsp;  </div></td>
<td > <div align="center">&nbsp;<? echo $rs_1[rec_status]?></div></td>
<td > <div align="center">&nbsp;<? if($rs_1[p_date] =='1111-11-11' or $rs_1[p_date] =='') echo "";
 else echo date_2($rs_1[p_date]);?></div></td>
</tr>
 <? 
	}
?>
