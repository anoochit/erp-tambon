...
<table  width="80%"   border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <td  height="28" colspan="14"  ><u><div  align="left">&nbsp;<strong>รายงาน(ป.32)&nbsp;&nbsp;หมู่ที่ : <?=$HOCODE." " .$honame?>&nbsp;เขต : <?=$ZID." " .$Zname?>&nbsp;&nbsp;ประจำวันที่ <?=date_2($p_date)?></strong></div>
        </u>
		</td>
      </tr>
  </table>
...
$i=0;
				...
				if ($rs_1[monthh] == '10' or $rs_1[monthh] == '11' or $rs_1[monthh] == '12' ) {
				 ...
				 $sql2.=  " or myear < '" .$year. "') and rec_status is not null and rec_status <> 'ยกเลิก' order by mcode,rec_id ";			
...
<table width="100%" align="center" cellspacing="0"  cellpadding="0" border="1" bordercolor="#666666">
<tr class="header">
      <td  height="28" colspan="14"><div  align="left">&nbsp;รายงานภาษีขาย / รายงานป.32 [ปกติ]&nbsp;&nbsp;หมู่ที่ : <?=$HOCODE." " .$honame?>&nbsp;ประจำเดือน : <?=mounth3($month)?>&nbsp;<?=$year?></div></td>
          </tr>
  <tr class="body1"  bgcolor="#DEE4EB">
        <td width="10%"  height="31" align="center"><strong>ลำดับที่</strong></td>
		...
		<td width="18%"  height="31" align="center"><p><strong>เลขที่ใบเสร็จ</strong></p>		  </td>
         <td width="15%"  align="center"><strong>เงินตามบิล</strong></td>
		 <td width="17%"  align="center"><strong>เงินที่เก็บได้</strong></td>
		 <td width="15%"  align="center"><strong>เงินที่ค้างชำระ</strong></td>
         </tr>
<?
$Y=0;
$a=0;
...
function Find_Remain($mcode,$year,$month){
        ...
        $sql =  "select sum(sum_m)as total  from meter m  where  mid(mcode,1,2) like '" .$HOCODE."' and mid(mcode,3,1) like '" .$ZID."'
		 and (((rec_id = '' or rec_id is  null or p_date ='1111-11-11') and rec_status = 'ค้างชำระ' ) or  p_date >'".$p_date_1."' ) and  ";
		$ex = substr($month,0,1);
		if($ex =='0') $monthh = substr($month,1);	
		else $monthh = $month;	 
...
function Find_SumPay($mcode,$year,$month){
		...
		$p_date_1 = ($year-543)."-".$month."-"."31";
		if($month == 12){ 	// ถ้าเป็นเดือนธันวาคม
				$MMM = ($year - 542) . "-01-%"; 
		}else  {
				$MMM = ($year - 543). "-";
