...
var minYear = 2006, maxYear = 2020;

document.write('<table id="tblCalender" cellpadding="1" cellspacing="1" border="0"'
	+'style="position:absolute;visibility:hidden;font-family:Tahoma;color:#8dc63f;font-size:x-small;font-weight:normal;border:thin solid #7da7d8;background-color:#7da7d8;">');
document.write('<tr>');
document.write('<td colspan="6" align="center">');

document.write('<select name="optCalenderMonth" onchange="updateMenu();" style="font-size:x-small;font-weight:normal;color:#8dc63f;">');
for (i=0; i<12; i++){
	document.write('<option value="' + i + '">' + getMonths(i) + '</option>');
}
document.write('</select>&nbsp;&nbsp;');

document.write('<select name="optCalenderYear" onchange="updateMenu();" style="font-size:x-small;font-weight:normal;color:#8dc63f;">');
...
document.write('<td align="right" valign="top"><span style="cursor:hand;font-size:8px;text-align:center;color:#FFFFFF;" '
...
function popupCalender(field){
	...
	with (document.getElementById('tblCalender')){
		...
		//style.top = event.clientY +6;
		//style.left = event.clientX -210;
...
function calOut() {
	if (!isNaN(parseInt(event.srcElement.innerText))) {
		event.srcElement.style.color = "#8dc63f";
	}
}
function calClick() {
	if (!isNaN(parseInt(event.srcElement.innerText))) {
...
function closeOver() {
	event.srcElement.style.backgroundColor = "#8dc63f";
	event.srcElement.style.color = "#FFCC00";
}
function closeOut() {
	event.srcElement.style.backgroundColor = "#31659C";
