...
<table  width="98%" border="0" align="center" cellpadding="0" cellspacing="0" >
  <tr class="bmText">
	...
	<table width="100%" border="1" align="center" cellpadding="0" cellspacing="0"  bordercolor="#000000">
      <tr class="lgBar">
        <td  height="28" colspan="14" ><div  align="left">&nbsp;&nbsp;<strong>รายงานสรุปยอดเก็บได้สำนักงาน [ป.32]&nbsp;&nbsp;หมู่ที่ : <?=$HOCODE." " .$honame?>&nbsp;ประจำเดือน : <?=mounth3($month)?>&nbsp;<?=$year?>
        </strong> </div></td>
      </tr>
      <tr class="bmText"  >
...
while($rs_1=mysql_fetch_array($result_1)){
    ...
    $hocode1 = substr($rs_1[z_id],0,1);
	if($z_id1 =='0') $z_id = substr($rs_1[z_id],1);	
	else $z_id = $rs_1[z_id];	
?>

<td width="12%"  height="31" align="center" ><strong>
หมู่ที่ <?=$z_id?>
</strong></td>
<? }
?>
...
$sql1.= " and p_date  like  '" .$p_date_1. "%'  and z_id = mid(mcode,3,1) and hocode = mid(mcode,1,2) and ";
...
and mid(mcode,1,2) = ".$HOCODE." group by p_date order by p_date";
...
<? 
$sql2="select  if(sum(sum_m)is null,'',sum(sum_m))as sumtotal,z_id,h.hocode 
from (house h,zone z)left join meter on z_id = mid(mcode,3,1) 
and z.hocode = mid(mcode,1,2) 
and rec_id is not null and rec_id <> ''  and p_date = '" .$rs1[p_date]. "'
...
from (house h,zone z)left join meter on z_id = mid(mcode,3,1) 
and z.hocode = mid(mcode,1,2) 
and rec_id is not null and rec_id <> '' 
