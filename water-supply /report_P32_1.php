...
<tr  class="header" bgcolor="#DEE4EB" >
  ...
  $Mold  = 0;
   $Summoney  = 0;
    $sql_1=" select rec_date,monthh,myear,moo, sum(sum_m) as money from meter r ,q_water m 
...
$bg ='#e8edff';
			...
			$sql=" select rec_date,monthh,myear,mid(m.mcode,1,2) as m1, sum(sum_m) as money1 from meter r ,q_water m 
        where  r.mcode = m.mcode and p_date = '".$p_date."' 
...
$i++;
 ...
 ?> </td>
 <td  align="center">&nbsp;<?=number_format($rs[money1],2);  ?></td>
 <td  align="center">&nbsp;
 <?
 if($rs_1[monthh]==$Mold and $rs_1[money]==$Summoney) {
   echo ""; 
 }else{ 
		echo number_format($rs_1[money],2);
}
  ?>
 </td>
</tr>
...
<tr class="body1"  bgcolor="#DEE4EB">
 <td  height="25"  align="center"><strong>รวมจำนวนเงินที่ส่ง</strong></td>
  ...
  <td  height="25"  align="center">&nbsp;</td>
   <td  align="center">&nbsp;<? echo number_format($stotal,2);?></td>
 <td  align="center">&nbsp;</td>
 </tr>
        </table>
	  </td>
