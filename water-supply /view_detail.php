...
$honame = $rsM[honame];
if($ZID <>'') {
$sqlZ="select z_name  from zone  where hocode = ".$hocode." and z_id = ".$ZID." "; 
 //echo  $sqlZ."<br>";
$resultZ = mysql_query($sqlZ);
if ($resultZ)
$rsZ=mysql_fetch_array($resultZ) ;
		$Zname = "  เขต : ".$ZID." ".$rsZ[z_name];
}else{
		$Zname = "";
}
?>
  ...
  <tr  class="header">
      <td  height="35" colspan="14" ><div  align="left" >ข้อมูลบิลค้างชำระ&nbsp;&nbsp;หมู่ที่ : <?=$hocode." " .$honame?> &nbsp;<?=$Zname?> &nbsp;ประจำเดือน : &nbsp;
        <?=mounth3($month)."  ".$year?>
     </div></td>
          </tr>
 <tr >
...
or  p_date >'".$p_date_1."' ) ";
if($hocode <>'') $sql_1 .=  "and mid(m.mcode,1,2) = '" .$hocode."' ";
if($ZID <>'') $sql_1 .=  "and mid(m.mcode,3,1) = '" .$ZID."' ";
$ex = substr($month,0,1);
