...
?>
 <tr class="bmText" >
		...
		<td  height="25"   align="right"style="border: #7292B8 1px solid;" ><font size="1"></font><font size="1"><strong>
		  <?=number_format($rs2[sumtotal2],2)?></strong>&nbsp;&nbsp;</font></td>
<? 
$sql_1="select mid(rec_date,1,4) as myear ,myear as Y1,monthh, mid(m.mcode,1,2) as moo ,sum(sum_m )  as sum1,hocode
from meter m, member mb, q_water q 
Where m.mcode = q.mcode and q.mem_id = mb.mem_id 
and (((rec_id = '' or rec_id is  null or p_date ='1111-11-11') and rec_status = 'ค้างชำระ')";
		if ($month == '10' or $month == '11' or $month == '12' ) {
...
mid(rec_date,1,4)) as myear,myear as Y2,if(sum(sum_m)is null,'',sum(sum_m))as sumtotal
...
?>
<a href="report_Monthly8_2.php?hocode=<?=$rs1[hocode]?>&month=<?=$month?>&year=<?=$year?>&month2=<?=$rs_1[monthh]?>&year2=
<?=$rs_1[Y1]?>" target="_blank"><? echo number_format($rs1[sumtotal],2)?></a>
<? }else{?>
<? }?>
<? 		
		}
