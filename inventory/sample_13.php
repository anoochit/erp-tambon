...
if($Go <> ''){
		...
		$sql_order = "INSERT INTO useful (year, item , useful ,c_id ,remark  ) 
		VALUES('$year','$item','$useful','$c_id'  ,'$remark')";
		echo $sql_order."<br>";
...
if($Go1 <> ''){
		...
		$sql_order = "UPDATE useful SET year = '$year' ,useful= '$useful', item = '$item',remark ='$remark'
		where u_id = '$u_id' ";
...
<? }elseif($tab=='edit'){
$sql = "SELECT * FROM useful WHERE u_id= $u_id ";
...
<?
	function find_max_m_id() {
		...
		$sql = "Select max(m_id) as max  from useful  ";
		$result = mysql_query($sql); 
