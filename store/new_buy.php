...
if($save <>''){
if(check_paper($num_id,$paper_id,$_SESSION[div_id]) <=0){
		$r_id = find_maxid();
		$sql_r = "INSERT INTO receive (r_id,num_id,shop_id,date_receive,come_in , paper_id, remark  ,div_id,shop_name ) 
		VALUES('$r_id','$num_id','','".date_format_sql($date_receive)."','$come_in' , '$paper_id' , '$remark'  ,'".$_SESSION[div_id]."','$shop_id')";
	...
	for($o=0;$o<=$pointer_array;$o++){
		...
		if($data[1] =='') $data[1] = $data[2];
				...
				//- - - -- - -- - -- - - - - - - - -บันทึกลง stock_card
				$k = explode(",",check_stock_1(date_format_sql($date_receive),$data[5] , $_SESSION[div_id]));
				// --------------------เพิ่มจำนวนหลังวันที่เพิ่ม----------------------
				$a_mount = $data[3] * $data[6];
				update_stock(date_format_sql($date_receive),$a_mount ,$data[5] , $_SESSION[div_id] );
				$amount_1 =  $k[0] + $a_mount;
				$sql_add2 = "INSERT INTO stock_card ( s_date , s_time , num_id , s_type , p_id , amount ,remain ,div_id ) 
				VALUES('".date_format_sql($date_receive)."',NOW() , '$rd_id1' , 'R' ,'$data[5]','$a_mount', '$amount_1' ,'".$_SESSION[div_id]."')";
				$result_add2 = mysql_query($sql_add2); 
			
				unset($dd[$o]); 
				session_unregister("dd[$o]");
			} // end  for($m=0;$m<$pointer_array;$m++){
		} // end if($data[1]<>''){
	unset($seri); 
	session_unregister("seri");
	echo " <center><br><br>ระบบกำลังบันทึกข้อมูล กรุณารอสักครู่  <br><br>";
	echo "<meta http-equiv='refresh' content='1; url=?action=view_detail_1&r_id=$r_id'>";
	echo "</center>";
	}else{
		...
		<table width="86%" border="0" cellspacing="1" cellpadding="1">
		 ...
		 <input type="text" name="shop_id" size="40" value="<?=$shop_id?>" >
			  ...
			  <tr>
                ...
                <td><textarea cols="40" rows="3" name="remark"><?=$remark?></textarea>
 </td>
              </tr>      
</table></td>
			</tr>
...
<a href="#" onClick="javascript:popup('add_new.php?action=add&OK=OK','',550,350);window.navigate('?action=new_buy');">เพิ่มรายการ
</a>
</center>
<br>
		<br>
...
<td style="border: #000000 1px solid;" align="left" height="28"><font size="2">&nbsp;<? echo $data[0]; ?></font></td>
                                                  ...
                                                  <td style="border: #000000 1px solid;" ><p align="center"><b><span style="font-size:11pt;"><font face="MS Sans Serif">
                                                   ...
                                                   <a href="?action=new_buy&del=del&array1=<?=$m?>"  onclick="return godel()">
...
function find_maxid() {
	...
	function check_paper($num_id,$paper_id,$div_id){
		$sql = "Select *  from receive  where paper_id = '$paper_id'  and div_id = '$div_id' ";
		$result = mysql_query($sql); 
