  ...
  <tr>
    <td  colspan="2" style="border: #7292B8 1px solid;"  >
		<table width="100%" border="0" cellspacing="1" cellpadding="1"  bgcolor="#F4F7F9">
 		 	<tr  class="lgBar1" >
  		  		<td  height="25" colspan="4" ><div class="style1">รายงานประวัติการลงโทษ</div></td>
  			</tr>
			    <tr >
...
<table width="100%" cellpadding="0" cellspacing="0" border="1" align="center"  bordercolor="#FFFFFF">
	...
	<tr  bgcolor="#60c0ff">
		...
		<td  align="center" height="25" width="216"> ชื่อ - สกุล</td>
		<td  align="center" height="25" width="260"> ตำแหน่ง</td>
		<td align="center" height="25" width="166"> วันที่</td>
		<td   align="center" height="25" width="156">บทลงโทษ</td>
...
<?
if($go_search  <> ""){
$sql="SELECT p.*,l.* ,w.per_id,ps.ps_tname_item,w.user_id as user_id1,w.position FROM (person  p, ps_tname_ref ps , law l)
...
if($date_begin1 <> '' and $date_begin2 <>''){
	$sql .= " AND l.date_add >= '".date_format_sql($date_begin1)."' AND l.date_add <= '".date_format_sql($date_begin2)."'  ";
}
if($div_id <>''){
...
$sql .= "   group by l.ll_id  order by  w.per_id ";
$Per_Page =10;
if(!$Page){$Page=1;}
$Prev_Page = $Page-1;
$Next_Page = $Page+1;
...
<tr bgcolor="<? echo $bg?>" >
		...
		<td   align="left"height="25" width="216" >&nbsp; <a href="index.php?action=personnel_story&user_id=<? echo $rs["user_id1"]?>"  target="_blank"><? 

		if($rs[ps_tname_id] <>'00') 
		 echo $rs[ps_tname_item]." ";
		  else " ";
		echo $rs["name"]?></a></td>
		<td    align="left"height="25" width="260">&nbsp; <?=$rs[position]?></td>
		<td   align="center" height="25" width="166">&nbsp; <? if($rs[date_add] <>'0000-00-00')echo date_2($rs[date_add]);else echo ""; ?></td>
		<td   align="center" height="25" width="156">&nbsp; <?=find_wrong_name($rs[ps_wrong_id])?></td>
...
<? /* สร้างปุ่มย้อนกลับ */
if($Prev_Page) 
echo " <a href='?action=report_law&Page=$Prev_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'><< ย้อนกลับ </a>";
for($i=1; $i<$Num_Pages; $i++){
if($i != $Page)

echo "[<a href='?action=report_law&Page=$i&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'>$i</a>]";
else 
echo "<b> $i </b>";
}
/*สร้างปุ่มเดินหน้า */
if($Page!=$Num_Pages)
echo "<a href ='?action=report_law&Page=$Next_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'> หน้าถัดไป>> </a>";
...
<input type="submit" name="print" value=" พิมพ์" onclick="window.open('report_law_print.php?date_begin1=<?=$date_begin1?>&date_begin2=<?=$date_begin2?>&div_id=<?=$div_id?>&sub_id=<?=$sub_id?>')"/>
</center>
<?
function find_w_id(){
