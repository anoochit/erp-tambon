  ...
  <tr>
    <td  colspan="2" style="border: #7292B8 1px solid;"  >
		<table width="100%" border="0" cellspacing="1" cellpadding="1"  bgcolor="#F4F7F9">
 		 	<tr  class="lgBar1" >
  		  		<td  height="25" colspan="4" ><div>รายงานประวัติปฎิบัติราชการใช้กฎอัยการศึก</div></td>
  			</tr>
			    <tr >
...
<tr bgcolor="<? echo $bg?>" >
		...
		<td   align="left"height="25" width="152" >&nbsp; <a href="index.php?action=personnel_story&user_id=<? echo $rs["user_id1"]?>"  target="_blank"><? 
...
<? /* สร้างปุ่มย้อนกลับ */
if($Prev_Page) 
echo " <a href='?action=report_dep_sec&Page=$Prev_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'><< ย้อนกลับ </a>";
for($i=1; $i<$Num_Pages; $i++){
if($i != $Page)

echo "[<a href='?action=report_dep_sec&Page=$i&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'>$i</a>]";
else 
echo "<b> $i </b>";
}
/*สร้างปุ่มเดินหน้า */
if($Page!=$Num_Pages)
echo "<a href ='?action=report_dep_sec&Page=$Next_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'> หน้าถัดไป>> </a>";
...
<input type="submit" name="print" value=" พิมพ์" onclick="window.open('report_dep_sec_print.php?date_begin1=<?=$date_begin1?>&date_begin2=<?=$date_begin2?>&div_id=<?=$div_id?>&sub_id=<?=$sub_id?>')"/>
</center>
<?
