  ...
  <tr>
    <td  colspan="2" style="border: #7292B8 1px solid;"  >
		<table width="100%" border="0" cellspacing="1" cellpadding="1"  bgcolor="#F4F7F9">
 		 	<tr  class="lgBar1" >
  		  		<td  height="25" colspan="4" ><div>รายงานการฝึกอบรม</div></td>
  			</tr>
			    <tr >
                <td width="48" class="style_h"><div> วันที่ </div></td>
                <td width="281" ><div> <input name="date_begin1" type="text" id="date_begin1" 
	value="<? if($date_begin1=='') echo date("d/m/Y"); else echo $date_begin1;?>"  size="10"/>
				  ...
				  <td width="58" class="style_h"><div> ถึงวันที่ </div></td>
                <td width="269" ><div> <input name="date_begin2" type="text" id="date_begin2" 
...
<table width="100%" cellpadding="0" cellspacing="0" border="1" align="center"  bordercolor="#FFFFFF">
	...
	<tr  bgcolor="#60c0ff">
		...
		<td align="center" height="25" width="150"> วันที่ฝึกอบรม</td>
		<td   align="center" height="25" width="124">ถึงวันที่</td>
		<td   align="center" height="25" width="153"> สถานที่	</td>
...
<tr bgcolor="<? echo $bg?>" >
		...
		<td align="left" height="25" width="135">&nbsp; <? 
		echo $rs[per_id]?></td>
		
		<td   align="left"height="25" width="153" >&nbsp; <a href="index.php?action=personnel_story&user_id=<? echo $rs["user_id1"]?>"  target="_blank"><? 
		...
		<td   align="center" height="25" width="150">&nbsp; <? if($rs[date_begin] <>'0000-00-00')echo date_2($rs[date_begin]);else echo ""; ?></td>
		<td   align="center" height="25" width="124">&nbsp; <? if($rs[date_end] <>'0000-00-00')echo date_2($rs[date_end]);else echo "";?></td>
		<td  align="left" height="25" width="153">&nbsp; <? echo $rs[place]?></td>
...
<? /* สร้างปุ่มย้อนกลับ */
if($Prev_Page) 
echo " <a href='?action=report_train&Page=$Prev_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'><< ย้อนกลับ </a>";
for($i=1; $i<$Num_Pages; $i++){
if($i != $Page)

echo "[<a href='?action=report_train&Page=$i&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'>$i</a>]";
else 
echo "<b> $i </b>";
}
/*สร้างปุ่มเดินหน้า */
if($Page!=$Num_Pages)
echo "<a href ='?action=report_train&Page=$Next_Page&go_search=$go_search&date_begin1=$date_begin1&date_begin2=$date_begin2&div_id=$div_id&sub_id=$sub_id'> หน้าถัดไป>> </a>";
...
<input type="submit" name="print" value=" พิมพ์" onclick="window.open('report_train_print.php?date_begin1=<?=$date_begin1?>&date_begin2=<?=$date_begin2?>&div_id=<?=$div_id?>&sub_id=<?=$sub_id?>')"/>
</center>
<?
function find_w_id(){
