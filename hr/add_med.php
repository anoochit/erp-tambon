...
if($Go <> ''){
		...
		$sql = "INSERT INTO medal (user_id , date_add,ps_medid, remark) 
		VALUES('$user_id' , '".date_format_sql($date_add)."' , '$ps_medid' , '$remark' )";
...
if($Go_1 <> ''){
		...
		$sql_order = "UPDATE medal SET date_add ='".date_format_sql($date_add)."' ,ps_medid ='$ps_medid' ,remark ='$remark' 
		where m_id ='$m_id' ";
		echo $sql_order."<br>";
  ...
  <tr>
    <td  colspan="2" style="border: #ff9966 1px solid;"  >
		<table width="100%" border="0" cellspacing="1" cellpadding="1">
 		 	<tr >
	  		  <td  height="25" bgcolor="#ff9966" class="tbETitle">&nbsp; เพิ่ม ข้อมูลเครื่องราชย์</td>
  			</tr>
			<tr>
			<td><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#ffcc99" class="calDay" name="add">
			<tr class="tbETitle_1">
   <td><span class="style5">&nbsp;ประเภทเหรียญ</span></td>
    <td><div class="style6" >
		<?
			$query="select *  from ps_medal_ref order by ps_medid ";
			$result=mysql_query($query);
			?>
            <select name="ps_medid">
              <?
			while($d =mysql_fetch_array($result)){
				
	?>
              <option value="<? echo $d[ps_medid];?>"
		<?
		if($ps_medid == $d[ps_medid] ) echo "selected";
		?>
		><? echo $d[ps_meditem];?></option>
              <? }?>
            </select>
...
<? }elseif($action=='edit'){?>
<?
$sql = "SELECT * FROM medal WHERE m_id= $m_id";
$result=mysql_query($sql);
...
<table  border="0" align="center" cellpadding="1" cellspacing="1"  width="98%">
  <tr>
    <td  colspan="2" style="border: #ff9966 1px solid;"  >
		<table width="100%" border="0" cellspacing="1" cellpadding="1">
 		 	<tr >
	  		  <td  height="25" bgcolor="#ff9966" class="tbETitle">&nbsp; แก้ไข ข้อมูลเครื่องราชย์</td>
  			</tr>
			<tr>
			<td><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#ffcc99" class="calDay" name="add">
			<tr class="tbETitle_1">
   <td><span class="style5">&nbsp;ประเภทเหรียญ</span></td>
	...
	?>
              <option value="<? echo $d[ps_medid];?>"
		<?
		if($rs[ps_medid] == $d[ps_medid] ) echo "selected";
		?>
		><? echo $d[ps_meditem];?></option>
              <? }?>
            </select>
